...
export function validateCompilableDefinition(
  ...
  | {
      ok: true;
      definition: ReturnType<typeof parseRuleDef>;
      inputSchema: ReturnType<typeof parseInputSchema>;
    }
  | { ok: false; error: DefinitionValidationError } {
  try {
...
export function defaultDraftDefinition(type: 'simple' | 'decision_table'): unknown {
  if (type === 'simple') {
    return { type: 'simple', when: { logic: 'and', items: [] }, then: [] };
  }
  return {
    type: 'decision_table',
    hitPolicy: 'first',
